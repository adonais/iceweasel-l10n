...
containers-disable-alert-desc =
    { $tabCount ->
        [one] 如果您現在禁用容器分頁，將有 { $tabCount } 個容器分頁被關閉。您確實要禁用容器分頁嗎？
       *[other] 如果您現在禁用容器分頁，將有 { $tabCount } 個容器分頁被關閉。您確實要禁用容器分頁嗎？
    }
containers-disable-alert-ok-button =
    { $tabCount ->
        [one] 關閉 { $tabCount } 个容器分頁
       *[other] 關閉 { $tabCount } 个容器分頁
    }
...
addressbar-locbar-showtrendingsuggestions-option =
    .label = 顯示熱門搜尋建議
    .accesskey = t
addressbar-quickactions-learn-more = 詳細瞭解
...
content-blocking-reload-description = 需要重新載入分頁才能應用變更。
...
collection-backlogged-crash-reports-with-link = 允許 { -brand-short-name } 以您的身分自動回報錯誤報告<a data-l10n-name="crash-reports-link">了解更多資訊</a>
    .accesskey = c
collection-backlogged-crash-reports = 允許 { -brand-short-name } 代您發送積壓的崩潰報告
    .accesskey = c
