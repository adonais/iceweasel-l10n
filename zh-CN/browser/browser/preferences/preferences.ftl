...
sync-engine-addons =
    .label = 附加组件
    .tooltiptext = 用于 { -brand-short-name } 桌面版的扩展和主题
    .accesskey = A
...
iceweasel-taskbartabs-description = Mozilla 开发的新功能就是为了让人们禁用它。
iceweasel-taskbartabs-warning1 = 把标签固定到操作系统任务栏, 有什么意义?
...
iceweasel-libportable-ghproxy-description = gh-proxy加速Iceweasel更新以及脚本下载, 建议中国国内用户开启。
iceweasel-libportable-ghproxy-warning1 = gh-proxy提供免费服务, 可能面临不稳定的问题。
...
iceweasel-libportable-mousegestures-description = 支持luajit脚本，你可按自己的要求，自行修改鼠标手势脚本。
iceweasel-libportable-ucaddons-description = 在附加组件页面增加UC脚本管理菜单。
...
mouse_gestures_aria2_webui = 访问 Aria2 WebUI
mouse_gestures_quick_shutdown = 关闭浏览器
mouse_gestures_open_chrome = 打开Chrome目录
mouse_gestures_previous_page = 上一页
...
reload_download_finish = 读取 Upcheck 支持的下载工具完成，请选择你喜欢的下载工具
download_through_upcheck = 使用 Upcheck 下载
download_by_default = 使用默认工具下载
